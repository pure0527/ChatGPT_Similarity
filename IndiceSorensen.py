...
def sorensen(a, b):
  #Calcular y retornar el coeficiente de Sorensen Dice entre dos conjuntos
  intersection = np.logical_and(a, b)
  similarity = (2 * intersection.sum()) / (a.sum() + b.sum())
  return similarity
...
while k < len(abstract_gt):
  ...
  #Calcular la matriz de distancias utilizando el coeficiente de Sorensen Dice
  i = 0
  while i < matriz_title.shape[1]:
    j = 0
    while j < matriz_title.shape[1]:
      matriz_distancia[i][j] = sorensen(matriz_title[:,i], matriz_title[:,j])
      j+=1
    i+=1
  #Incrementar el contador y almacenar el resultado
  k+=1
  matriz_distancia = matriz_distancia
  total_sorensen.append(matriz_distancia[0][1])
#Crear DataFrame y guardar resultados en un archivo CSV
df = pd.DataFrame(total_sorensen)
df.to_csv("results_sorensen.csv")