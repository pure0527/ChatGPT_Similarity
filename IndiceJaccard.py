...
def jaccard(a, b):
  #Calcular y retornar el coeficiente de Jaccard entre dos conjuntos
...
while k < len(abstract_gt):
  ...
  #Calcular la matriz de distancias utilizando el coeficiente de Jaccard
  i = 0
  while i < matriz_title.shape[1]:
    j = 0
    while j < matriz_title.shape[1]:
      matriz_distancia[i][j] = jaccard(matriz_title[:,i], matriz_title[:,j])
      j+=1
    i+=1
  #Incrementar el contador y almacenar el resultado
  k+=1
  matriz_distancia = matriz_distancia
  total_jaccard.append(matriz_distancia[0][1])
#Crear DataFrame y guardar resultados en un archivo CSV
df = pd.DataFrame(total_jaccard)
df.to_csv("results_jaccard.csv")